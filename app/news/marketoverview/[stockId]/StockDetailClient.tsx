...
export default function StockDetailClient({ stockId }: StockDetailClientProps) {
  const [stock, setStock] = useState<StockData | null>(null);
  const [activeExchange, setActiveExchange] = useState<"NSE" | "BSE">("NSE");

  const demoStocks: StockData[] = useMemo(
    () => [
      { id: "AAPL", name: "Apple Inc.", exchange: "NSE" },
  ...
  useEffect(() => {
    const stock = demoStocks.find((s) => s.id === stockId);
    if (stock) {
      setStock(stock);
    }
  }, [stockId, demoStocks]);
